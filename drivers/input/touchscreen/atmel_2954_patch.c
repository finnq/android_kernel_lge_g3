				...
				if (check_cnt != NULL) {
					*check_cnt = *check_cnt + 1;
					...
					if (*check_cnt == ptline->check_cnt) {
						test_action = true;
						__mxt_patch_debug(data, "STAGE:%d TEST %d MATCHED", data->patch.cur_stage, ptline->test_id);
...
int mxt_patch_event(struct mxt_data *data, u8 event_id)
{
	...
	if (data->is_probing) {
		TOUCH_PATCH_INFO_MSG("%s Touch IC is still probing.. do not update Patch Set!!\n", __func__);
		return 1;
	}
	...
	if(index >= MXT_T100_SCREEN_MESSAGE_NUM_RPT_ID){
		...
		for(i=0; i< MXT_MAX_NUM_TOUCHES; i++){
			if(tch_slot.detect[i]){
				...
				if(tch_slot.type[i] == MXT_T100_TYPE_LARGE_TOUCH){
					large_cnt++; //20131227
				}
			}
		}
		tsrc.stylus_cnt = stylus_cnt;
		tsrc.large_cnt = large_cnt; //20131227
#endif

		switch (touch_type)	{
			case MXT_T100_TYPE_PATCH_FINGER:
	//		case MXT_T100_TYPE_PASSIVE_STYLUS:
	//		case MXT_T100_TYPE_GLOVE_TOUCH:
	//		case MXT_T100_TYPE_LARGE_TOUCH: //20131227
				x = msg[1] | (msg[2] << 8);
				y = msg[3] | (msg[4] << 8);
